...
//!   - Política de firma       : Facturae v3.1 (hash de política en SHA1 según especificación)
...
pub fn sign_xml_xades(unsigned_xml: &str, parent_hwnd: Option<isize>) -> AppResult<String> {
    ...
    let doc_c14n = canonicalize_c14n_1_0(strip_xml_declaration(unsigned_xml))?;
    let doc_digest_b64 = sha256_b64(&doc_c14n);

    let mut signing_time = String::new();
    let mut cert_sha1_b64 = String::new();
    let mut issuer_dn = String::new();
    let mut serial_decimal = String::new();
    let mut signed_props_xml = String::new();
    let mut signed_info_xml = String::new();

    // ── 1. Seleccionar certificado y firmar SignedInfo mediante Windows CNG ──
    #[cfg(target_os = "windows")]
    let material = win_sign::pick_cert_and_sign_with(parent_hwnd, |cert_der| {
        let x509 = x509_cert::Certificate::from_der(cert_der)
            .map_err(|e| AppError::Certificate(format!("Certificado inválido: {e}")))?;

        issuer_dn = x509.tbs_certificate.issuer.to_string();
        serial_decimal = bytes_to_decimal(x509.tbs_certificate.serial_number.as_bytes());
        cert_sha1_b64 = sha256_b64(cert_der);
        signing_time = chrono::Utc::now()
            .format("%Y-%m-%dT%H:%M:%SZ")
            .to_string();

        signed_props_xml = build_signed_properties(
            signed_props_id,
            &signing_time,
            &cert_sha1_b64,
            &issuer_dn,
        ...
        let sp_c14n = canonicalize_c14n_1_0(&signed_props_xml)?;
        let sp_digest_b64 = sha256_b64(&sp_c14n);

        signed_info_xml = build_signed_info(&doc_digest_b64, signed_props_id, &sp_digest_b64);
        canonicalize_c14n_1_0(&signed_info_xml)
    })?;

    #[cfg(not(target_os = "windows"))]
    let material: CertMaterial = {
        let _ = parent_hwnd;
        let _ = &mut signing_time;
        let _ = &mut cert_sha1_b64;
        let _ = &mut issuer_dn;
        let _ = &mut serial_decimal;
        let _ = &mut signed_props_xml;
        let _ = &mut signed_info_xml;
    ...
    // ── 2. Montar el bloque <ds:Signature> ───────────────────────────────────
...
struct CertMaterial {
    cert_der: Vec<u8>,
    /// Bytes RSA-SHA256 PKCS#1v15 de la firma, sin codificar.
    signature: Vec<u8>,
}
...
mod win_sign {
    ...
    pub(super) fn pick_cert_and_sign_with<F>(
        parent_hwnd: Option<isize>,
        build_signed_info_c14n: F,
    ) -> AppResult<CertMaterial>
    where
        F: FnOnce(&[u8]) -> AppResult<Vec<u8>>,
    {
        use sha2::Digest as _;
        ...
        unsafe {
            ...
            let signed_info_c14n = build_signed_info_c14n(&cert_der)?;
            let hash_bytes: [u8; 32] = sha2::Sha256::digest(&signed_info_c14n).into();
            ...
            // 5. Firmar hash SHA256 con PKCS#1 v1.5
            let padding_info = BCRYPT_PKCS1_PADDING_INFO {
                pszAlgId: windows::Win32::Security::Cryptography::BCRYPT_SHA256_ALGORITHM,
            };
...
fn build_signed_properties(
    ...
    // Hash SHA1 de la política publicado en la especificación Facturae v3.1
    ...
    format!(
        r##"<xades:SignedProperties Id="{id}" xmlns:xades="{ns_xades}" xmlns:ds="{ns_ds}"><xades:SignedSignatureProperties><xades:SigningTime>{time}</xades:SigningTime><xades:SigningCertificate><xades:Cert><xades:CertDigest><ds:DigestMethod Algorithm="{alg_cert}"/><ds:DigestValue>{cert_digest}</ds:DigestValue></xades:CertDigest><xades:IssuerSerial><ds:X509IssuerName>{issuer}</ds:X509IssuerName><ds:X509SerialNumber>{serial}</ds:X509SerialNumber></xades:IssuerSerial></xades:Cert></xades:SigningCertificate><xades:SignaturePolicyIdentifier><xades:SignaturePolicyId><xades:SigPolicyId><xades:Identifier>{policy_url}</xades:Identifier></xades:SigPolicyId><xades:SigPolicyHash><ds:DigestMethod Algorithm="{alg_policy}"/><ds:DigestValue>{policy_sha1}</ds:DigestValue></xades:SigPolicyHash></xades:SignaturePolicyId></xades:SignaturePolicyIdentifier></xades:SignedSignatureProperties></xades:SignedProperties>"##,
        id = id,
        ns_xades = NS_XADES,
        ns_ds = NS_DS,
        time = signing_time,
...
fn build_signed_info(doc_digest_b64: &str, signed_props_id: &str, sp_digest_b64: &str) -> String {
    ...
    // xmlns:ds únicamente – sin xmlns:fe para evitar desvíos de contexto de
    // namespace al canonicalizar durante la verificación.
    const NS_DS: &str = "http://www.w3.org/2000/09/xmldsig#";
    // URI="" → documento entero. Las dos transforms permiten al verificador
    // reproducir exactamente el mismo byte-stream que se hasheó:
    //   1) enveloped-signature elimina el nodo <ds:Signature> antes del hash.
    //   2) xml-c14n normaliza el resultado igual que nosotros en Rust.

    format!(
        r##"<ds:SignedInfo xmlns:ds="{ns_ds}"><ds:CanonicalizationMethod Algorithm="{c14n}"/><ds:SignatureMethod Algorithm="{rsa_sha256}"/><ds:Reference Id="signed-data-ref" URI=""><ds:Transforms><ds:Transform Algorithm="{env_sig}"/><ds:Transform Algorithm="{c14n}"/></ds:Transforms><ds:DigestMethod Algorithm="{sha256}"/><ds:DigestValue>{doc_digest}</ds:DigestValue></ds:Reference><ds:Reference URI="#{sp_id}" Type="http://uri.etsi.org/01903#SignedProperties"><ds:DigestMethod Algorithm="{sha256}"/><ds:DigestValue>{sp_digest}</ds:DigestValue></ds:Reference></ds:SignedInfo>"##,
        ns_ds = NS_DS,
        c14n = C14N,
...
fn canonicalize_c14n_1_0(xml: &str) -> AppResult<Vec<u8>> {
    canonicalize(xml, C14nMode::Inclusive, None, &[] as &[&str])
        .map_err(|e| AppError::Internal(format!("Error canonicalizando XML (C14N): {e}")))
}
